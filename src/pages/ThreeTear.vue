 ...
 */
      ...
      const envLight = new THREE.AmbientLight({ color: 'white', intensity: 6 })
      stage.add(envLight)

      const pointLight = new THREE.PointLight({ color: 'white', intensity: 20 })
 ...
 */
      ...
      const init = () => {
        ...
        addEventListener('mousedown', (event) => down(event.clientX, event.clientY))
        addEventListener('touchstart', (event) => down(event.touches[0].clientX, event.touches[0].clientY))
        addEventListener('mousemove', (event) => move(event.clientX, event.clientY))
        addEventListener('touchmove', (event) => move(event.touches[0].clientX, event.touches[0].clientY))
